...
Everything the renderers consume comes from :meth:`SpatialHub.async_model`.
...
class SpatialHub:
    ...
    def async_start(self) -> None:
        ...
        _LOGGER.debug(
            "Spatial Hub listening (provider API v%s), %d provider(s) present",
            API_VERSION,
            len(self.providers),
        )
    ...
    def _notify(self, reason: str, provider_id: str) -> None:
        for listener in list(self._listeners):
            try:
                ...
                _LOGGER.exception("Spatial Hub listener raised")
