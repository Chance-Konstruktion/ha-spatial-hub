"""Spatial Hub -- the spatial bus for Home Assistant.
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if entry.options.get(CONF_PANEL, DEFAULT_PANEL):
        ...
        try:
            ...
            _LOGGER.exception("Spatial Hub could not register its panel")

    entry.async_on_unload(entry.add_update_listener(_async_options_updated))

    _LOGGER.info(
        "Spatial Hub ready with %d provider(s): %s",
        len(registrations),
...
async def _async_options_updated(hass: HomeAssistant, entry: ConfigEntry) -> None:
    hub: SpatialHub | None = hass.data.get(DATA_HUB)
    if hub is None:
        return
    hub.auto_areas = bool(entry.options.get(CONF_AUTO_AREAS, DEFAULT_AUTO_AREAS))
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if watcher is not None:
        watcher.async_stop()
    hub: SpatialHub | None = hass.data.pop(DATA_HUB, None)
    if hub is not None:
        hub.async_stop()
